...
// Skip these sheets
const SKIP_SHEETS = ['合計', '凡例', '売り上げ', 'グラス備品'];
...
function detectFormat(workbook, XLSX) {
  // Check first data sheet's header row
  for (const sheetName of workbook.SheetNames) {
    ...
    const header = data[0];
    const headerStr = header.map(h => String(h).trim()).join('|');
    // Format A: has "区分" and "生産者"
    if (headerStr.includes('区分') && headerStr.includes('生産者')) return 'burgundy';
    // Format B: has "店舗" and "正商品名"
    if (headerStr.includes('店舗')) return 'store';
    break;
  }
  // Fallback: check if any sheet has "その他アルコール" or "ソフトドリンク" → store format
...
export function parseInventoryExcel(workbook, XLSX) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    const mapping = Object.entries(SHEET_CATEGORY_MAP).find(([k]) =>
      trimmed === k || trimmed.includes(k) || k.includes(trimmed)
    );
    if (!mapping) continue;
